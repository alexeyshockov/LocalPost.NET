...
internal sealed class MessageSource(QueueClient client)
    : IBackgroundService, INamedService, IAsyncEnumerable<ConsumeContext<string>>
{
    ...
    public IAsyncEnumerator<ConsumeContext<string>> GetAsyncEnumerator(CancellationToken ct) =>
        ConsumeAsync(ct).GetAsyncEnumerator(ct);
    ...
    private async IAsyncEnumerable<ConsumeContext<string>> ConsumeAsync(
